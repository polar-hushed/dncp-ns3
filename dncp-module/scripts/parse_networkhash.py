...
#parser.add_argument("infile",type=argparse.FileType('r'),
 #                   help="the data file to analyize")
...
for file in glob.glob("*-*-*.networkhash"):
    ...
    if not os.path.exists('./'+topology+'.convergetime'):
        ff=open('./'+topology+'.convergetime','w')
        ff.close()
        
    
    found=False
    with open(topology+".convergetime",'r') as afile:
    ...
    with open(topology+".convergetime",'w') as afile:
        if found:
